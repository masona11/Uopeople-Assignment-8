...
public class TextMenu extends JMenu {
	...
	private JRadioButtonMenuItem leftJustify; // radio button for left justify
    private JRadioButtonMenuItem rightJustify; // radio button for right justify
    private JRadioButtonMenuItem centerJustify; // radio button for center justify
	...
	public TextMenu(DrawPanel owner) {
		...
		// Add Line Height Multiplier menu item
	    JMenuItem lineHeightMultiplier = new JMenuItem("Set Line Height Multiplier...");
	    lineHeightMultiplier.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent evt) {
	            String s = JOptionPane.showInputDialog(panel, "Set Line Height Multiplier:", panel.getTextItem().getLineHeightMultiplier());
	            if (s != null && s.trim().length() > 0) {
	                try {
	                    double multiplier = Double.parseDouble(s.trim());
	                    panel.getTextItem().setLineHeightMultiplier(multiplier);
	                    panel.repaint();
	                } catch (NumberFormatException e) {
	                    JOptionPane.showMessageDialog(panel, s + " is not a valid number.");
	                }
	            }
	        }
	    });
	    add(lineHeightMultiplier);
	}
